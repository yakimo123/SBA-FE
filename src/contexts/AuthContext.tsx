...
const extractRoleFromClaims = (claims: Record<string, unknown>): string => {
  const normalize = (value: string): string => value.replace(/^ROLE_/i, '').toUpperCase();
  ...
  if (accessToken) {
    ...
    if (decoded) {
      ...
      const userId =
        ...
        (typeof claims.sub === 'string' && /^\d+$/.test(claims.sub) ? Number(claims.sub) : 0);
      ...
      return {
        ...
        user: {
          userId,
          email: (typeof claims.email === 'string' ? claims.email : '') || (typeof claims.sub === 'string' && claims.sub.includes('@') ? claims.sub : ''),
          fullName:
            (typeof claims.fullName === 'string' ? claims.fullName : '') ||
            (typeof claims.name === 'string' ? claims.name : '') ||
            '',
          role,
          phoneNumber: (typeof claims.phoneNumber === 'string' ? claims.phoneNumber : undefined),
          // Backward compatible aliases
          name:
            (typeof claims.fullName === 'string' ? claims.fullName : '') ||
            (typeof claims.name === 'string' ? claims.name : '') ||
            '',
          phone: (typeof claims.phoneNumber === 'string' ? claims.phoneNumber : undefined),
          points: (typeof claims.rewardPoint === 'number' ? claims.rewardPoint : 0),
        } as AuthUser,
      };
    }
  }
