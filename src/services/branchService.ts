...
export const branchService = {
    ...
    async getBranchById(id: string | number): Promise<BranchResponse> {
        const response = await api.get<ApiResponse<BranchResponse>>(ENDPOINTS.BRANCH_BY_ID(id));
        return response.data.data;
    },

    async createBranch(data: CreateBranchRequest): Promise<BranchResponse> {
        const response = await api.post<ApiResponse<BranchResponse>>(ENDPOINTS.BRANCHES, data);
        return response.data.data;
    },

    async updateBranch(id: string | number, data: UpdateBranchRequest): Promise<BranchResponse> {
        const response = await api.put<ApiResponse<BranchResponse>>(ENDPOINTS.BRANCH_BY_ID(id), data);
        return response.data.data;
    },
    ...
    async updateBranchProductStock(branchId: string | number, productId: string | number, quantity: number): Promise<void> {
        await api.patch(
            ENDPOINTS.BRANCH_PRODUCT_STOCK(branchId, productId),
            null,
            { params: { quantity } }
