...
export const companyService = {
  ...
  search: async (
    ...
    page = 0,
    size = 20
  ): Promise<PageableResponse<CompanyResponse>> => {
    ...
    const response = await api.get<
      ApiResponse<PageableResponse<CompanyResponse>>
    >(`/api/v1/companies?${params.toString()}`);
    return response.data.data;
  },

  // Create new company (Admin/Legacy)
  ...
  // Register new company (Authenticated user flow)
  register: async (data: CreateCompanyRequest): Promise<CompanyResponse> => {
    const response = await api.post<ApiResponse<CompanyResponse>>(
